...
async function checkAllLinks(url) {
  ...
  const links = await page.evaluate(() => {
    // Only get links from header and footer, exclude body
    ...
    if (headerElement) {
      anchors = anchors.concat(Array.from(headerElement.querySelectorAll('a[href]')));
    }
    
    // Collect links from footer
    if (footerElement) {
      anchors = anchors.concat(Array.from(footerElement.querySelectorAll('a[href]')));
    }
    ...
    return anchors
      .map((a, index) => {
        ...
        const location = a.closest('header') ? 'Header' : 'Footer';
        return {
          href: a.href,
  ...
  for (let i = 0; i < links.length; i++) {
    const link = links[i];
    console.log(`[${i + 1}/${links.length}] Checking: ${link.href} (${link.location})`);
    ...
    try {
      ...
      // Close any interstitial popups before checking status
      await closeInterstitialPopups(page);
